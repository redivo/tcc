...
unsigned xatoi(int (*getc)(void), int base)
{
unsigned x,d, s;
x=0; s=0;
while((d = getc()) < '0') { s |= (d=='-'); ngscanf++; }
do	{
	if(d < '0') break;
	if(d >= 'a') d -= 'a' - 'A';
	d -= (d > '9')? 'A'-10: '0'; 
...
int va_scanf(int (*getc)(void), const char *formato, va_list va)
{
int nitem, c, c2, campo;
#ifdef FPSUPPORT
double *pd;
char buf[32];
#endif
...
while((c=*s++)) {
	...
	c = *s++;
	for(campo = 0; edigito(c); c=*s++) campo=10*campo+c-'0';
	while(c=='l' || c=='L' || c=='h') {c2 = c; c=*s++; }
	c = tolower(c);
	ngscanf = 0;
	switch(c) {
		case 'u': case 'd': case 'i':
			if(c2 == 'h') { psh = va_arg(va, short *); *psh = xatoi(getc,10); }
			else	{ pv = va_arg(va, int *); *pv = xatoi(getc, 10); }
			break;
		case 'x':
			pv = va_arg(va, int *); *(unsigned int *)pv = xatoi(getc, 16);
			break;
		case 'o': case 'q':
			pv = va_arg(va, int *); *(unsigned int *)pv = xatoi(getc, 8);
			break;
		case 's':
			if(campo == 0) campo=256;
			ps = ps2 = va_arg(va, char *);
			do	{
				c2 = getc();
				} while(c2 && c2 <= ' ');
			while(campo-- && c2 > ' ') {
...
int sgetc(void) { return *gp_buf++; }

/* O primeiro parametro e' o nome da funcao para pegar caracteres para passar para mscanf */
int mscanf(int (*getc)(void), const char *formato, ...)
{
va_list va;
va_start(va,formato);
return va_scanf(getc, formato, va);
}

/* Similar ao sscanf da biblioteca padrao */
#if 0
...
va_start(va,formato);
gp_buf=buf;
return va_scanf(sgetc, formato, va);
}
#endif 
